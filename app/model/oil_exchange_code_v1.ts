...
@Table({
    modelName: 'oil_exchange_code',
})
export class OilExchangeCodeModelV1 extends Model<OilExchangeCodeModelV1> {
    @PrimaryKey
    @AutoIncrement
    @Column({
        comment: '主键',
    ...
    public static async findByUserId(userId) {
        return OilExchangeCodeModelV1.findOne({
            where: {
                userId,
            },
        });
...
export default () => OilExchangeCodeModelV1;
